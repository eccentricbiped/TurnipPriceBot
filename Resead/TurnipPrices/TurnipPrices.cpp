...
namespace PREDICT
{
    ...
    struct PriceRange
    {
        ...
        PriceRange(uint16_t p) : min(p), max(p) {}
        PriceRange(uint16_t min_p, uint16_t max_p) : min(min_p), max(max_p) {}
        uint16_t min, max;
    };

    typedef vector<PriceRange> PriceRangeArray;
    
    struct Prices
    {
        Prices() : daisyMaePrice(0) {}
        uint16_t daisyMaePrice;
        vector<uint16_t> nookPrices;
    };
    ...
    PriceRangeArray GeneratePattern0(const Prices& prices)
    {
        PriceRangeArray result;

        return result;
    }
    
    PriceRangeArray GeneratePattern1WithPeak(const Prices& prices, uint16_t peakStart)
    {
        PriceRangeArray result;
        
        if (prices.nookPrices.size() == 0)
            return result;
    ...
    PriceRangeArray GeneratePattern1(const Prices& prices)
    {
        PriceRangeArray result;


        for (uint16_t peakStart = 3; peakStart < 20; ++peakStart)
        {
            PriceRangeArray curPeakPrices = GeneratePattern1WithPeak(prices, peakStart);
            AppendPriceRangeArray(result, curPeakPrices);
        }

        return result;
    }

    PriceRangeArray GeneratePattern2(const Prices& prices)
    {
        PriceRangeArray result;
    ...
    PriceRangeArray GeneratePattern3(const Prices& prices)
    {
		PriceRangeArray result;
    ...
    PriceRangeArray GeneratePossibilities(uint16_t BuyPrice, const vector<uint16_t>& NookPrices)
    {
		PriceRangeArray result;
